/*
 * Copyright (c) HiSilicon (Shanghai) Technologies Co., Ltd. 2023-2023. All
 * rights reserved. Description:  UART HAL Header.
 *
 * Create:  2023-02-17
 */
...
typedef enum {
  ...
  UART_ERROR_MAX_INFO_NUM /* The number of uart frame or parity error
                             information */
} uart_err_info_index;
...
void hal_uart_int_set(uart_bus_t bus, uint32_t reg, uint32_t int_id,
                      uint32_t val);
#endif
...
/**
 * @brief  Obtains the RX FIFO trigger level in bytes, as configured by
 * hal_uart_init_fifo
 * @param  uart Uart bus.
...
void hal_uart_enable_interrupt(uart_bus_t uart,
                               hal_uart_interrupt_t interrupt_type, bool val);
...
/**
 * @brief  Forces the pending excecution of the idle isr in an interrupt
 * context.
 * @param  uart Uart bus
 */
...
/**
 * @brief  Switch the TRX of the specified UART, uart_l default value PIN50: rx;
 * PIN51: tx. uart_h default value PIN36: tx; PIN37: rx. uart_m default value
 * PIN30: tx; PIN31: rx.
 * @param  uart The uart to switch.
 * @param  switch_trx false: the default uart trx status; true: reverse the uart
 * trx status.
 * @return The result of switched. true: switch the trx; false: switch to
 * default value.
 */
...
static inline bool hal_uart_get_cpr_shadow(uart_bus_t uart) {
  return ((uart_parameter_t)g_hal_uart_reg[uart]->uart_parameter)
