 ...
 *============================================================================*/
// Event mask definitions for interrupt->task signaling
#define DEMO_EVENT_UART_RX      (1U << 0)   // UART received data
#define DEMO_EVENT_SLE_RX       (1U << 1)   // SLE received data
#define DEMO_EVENT_SLE_TX_DONE  (1U << 2)   // SLE TX completed (reserved)
#define DEMO_EVENT_ALL          (DEMO_EVENT_UART_RX | DEMO_EVENT_SLE_RX | DEMO_EVENT_SLE_TX_DONE)

// Timeout for event wait (ms) - minimal timeout for lowest latency
#define DEMO_EVENT_TIMEOUT_MS   2

// Global event handle (extern declaration)
...
#define DEMO_SLE_CONN_INTV_MIN      0x10
#define DEMO_SLE_CONN_INTV_MAX      0x10
 ...
 * Task Configuration
 *============================================================================*/
// [LATENCY OPT] Higher priority = lower scheduling latency (LiteOS: smaller = higher)
...
typedef struct {
    ...
    volatile uint32_t sle_rx_timestamp_us;      // When SLE RX received data (for sender side latency)
